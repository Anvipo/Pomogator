...
class FieldView<ID: Hashable>: UIView, UITextFieldDelegate {
	...
	func textFieldDidChange(text: String) {
		assertionFailure("Implement this method")
	}
...
private extension FieldView {
	func setupStyle() {
		UIView.animate(withDuration: .defaultAnimationDuration) { [self] in
			if textField.isFirstResponder {
				titleLabel.textColor = Color.brand.uiColor
				backgroundColor = Color.brand.uiColor.withAlphaComponent(0.1)
			} else {
				titleLabel.textColor = item.titleColor
				backgroundColor = nil
			}
