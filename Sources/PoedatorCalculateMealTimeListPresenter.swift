...
extension PoedatorCalculateMealTimeListPresenter {
	...
	func didTapSaveButton() {
		userDefaultsFacade.calculatedMealTimeList = currentCalculatedMealTimeList
...
extension PoedatorCalculateMealTimeListPresenter: StringFieldItemDelegate {
	func stringFieldItem<ID: IDType>(
		...
		do {
			possibleNumber = try UInt(possibleString, format: assembly.numberOfMealTimesFormatStyle)
		} catch {
			return false
		}
	...
	func stringFieldItemFormattedString<ID: IDType>(
		...
		do {
			inputedNumberOfMealTimes = try UInt(currentText, format: assembly.numberOfMealTimesFormatStyle)
		} catch {
			return ""
		}

		return inputedNumberOfMealTimes.formatted(assembly.numberOfMealTimesFormatStyle)
	}
	...
	func stringFieldItemDidChangeString<ID: IDType>(
		...
		if newText.isEmpty {
			...
			do {
				inputedNumberOfMealTimes = try UInt(newText, format: assembly.numberOfMealTimesFormatStyle)
			} catch {
...
private extension PoedatorCalculateMealTimeListPresenter {
	func configureUI(changingItemID: PoedatorCalculateMealTimeListItemIdentifier?) {
		...
		if !currentCalculatedMealTimeList.isEmpty &&
			currentCalculatedMealTimeList != userDefaultsFacade.calculatedMealTimeList {
	...
	func configureCalculatedMealTimeListSection() {
		...
		if calculatedMealTimeList == currentCalculatedMealTimeList {
			return
		}

		notificationFeedbackGenerator.prepare()

		currentCalculatedMealTimeList = calculatedMealTimeList
		...
		if calculatedMealTimeListSection != nil {
			...
			calculatedMealTimeListSection?.headerItem = assembly
				.headerItem(
	...
	func calculateMealTimeList() -> [Date] {
		...
		if !isValid(numberOfMealTimes: inputedNumberOfMealTimes) {
			return []
		}
