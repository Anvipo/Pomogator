...
extension PoedatorYourMealTimeListPresenter {
	...
	) -> AnyPoedatorYourMealTimeListTableItem? {
...
private extension PoedatorYourMealTimeListPresenter {
		...
		let task = Task { [weak self] in
			...
			do {
				...
				if wasGranted {
					...
					await self.didNotGrantUserNotificationAuthorization()
				}
			} catch {
				assertionFailure(error.localizedDescription)
				await self.notificationFeedbackGenerator.notificationOccurred(.error)
			}
		}
	...
	func didGrantUserNotificationAuthorization() async {
		...
		do {
			...
			await didAddMealReminders(for: registeringMealTimeList)
		} catch {
			assertionFailure(error.localizedDescription)
