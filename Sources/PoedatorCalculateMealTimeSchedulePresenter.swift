...
private extension PoedatorCalculateMealTimeSchedulePresenter {
	...
	func convert(mealTime: Date) -> String {
		if calendar.isDateInYesterday(mealTime) {
			let timeString = mealTime.formatted(date: .omitted, time: .shortened)
			return String(localized: "Yesterday in \(timeString) meal time text")
		} else if calendar.isDateInToday(mealTime) {
			let timeString = mealTime.formatted(date: .omitted, time: .shortened)
			return String(localized: "Today in \(timeString) meal time text")
		} else if calendar.isDateInTomorrow(mealTime) {
			let timeString = mealTime.formatted(date: .omitted, time: .shortened)
			return String(localized: "Tomorrow in \(timeString) meal time text")
		} else {
