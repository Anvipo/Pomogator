...
extension PoedatorCalculateMealTimeListAssembly {
	var numberOfMealTimesFormatStyle: IntegerFormatStyle<UInt> {
		.number
	}
	...
	func parametersSection(
		...
		var numberOfMealTimesItem = Pomogator.stringFieldItem(
			...
			content: StringFieldItem.Content(
				...
				text: inputedNumberOfMealTimes?.formatted(numberOfMealTimesFormatStyle) ?? ""
			),
		...
		return (
			...
			try! PoedatorCalculateMealTimeListSection(
				...
				headerItem: Pomogator.headerItem(
					id: PoedatorCalculateMealTimeListHeaderItemIdentifier.parameters,
	...
	) -> PlainLabelHeaderItem<PoedatorCalculateMealTimeListHeaderItemIdentifier> {
		let text: String
